...
public class Game {
	...
	private void EndTurn() { //resets to standard defaults for turns then changes the active player
		gameStateOne = true; 
		currentTurn = currentTurn == PieceColor.WHITE ? PieceColor.WHITE : PieceColor.BLACK; // switch to white, expecting students to look up terniary operator
		gameStateTwo = false;
		checkUpGrade();
	}
	
	private void checkUpGrade() { //sees if a pawn can be upgraded to a king
		if(selected instanceof Pawn & // should be &&, & is lazy AND
		((selected.getColor() == PieceColor.WHITE || selected.getPosition().getY()==0) || // changed &&s to ||s
		(selected.getColor() == PieceColor.BLACK || selected.getPosition().getY()==8))) { // switched White and Black, should all be changed when spawned.
